defmodule EDA.Voice.Dave.IntegrationTest do
  ...
  describe "MLS event handling" do
    ...
    test "OP 21 PREPARE_TRANSITION is acknowledged", %{manager: manager} do
      {updated, replies} =
        Manager.handle_mls_event(manager, 21, %{"transition_id" => 1, "protocol_version" => 1})

      assert [%{op: 23, d: %{transition_id: 1}}] = replies
      assert updated.protocol_version == 1
    end

    test "OP 22 EXECUTE_TRANSITION executes the pending transition", %{manager: manager} do
      {manager, _} =
        Manager.handle_mls_event(manager, 21, %{"transition_id" => 7, "protocol_version" => 1})

      {updated, []} = Manager.handle_mls_event(manager, 22, %{"transition_id" => 7})
      assert updated.pending_transitions == %{}
      assert updated.last_transition_id == 7
    end
